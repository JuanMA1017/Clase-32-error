...
function draw() {
  ...
  for(var i=0; i<mbalas.length;i++){
    ...
    console.log("longitud de mbalas", mbalas.length)
  }
  cannonobj.display();
...
function colisiones(index){
  ...
  if(mbalas[index] !==undefined && mboats[i]!==undefined){
    console.log("indice de balas", mbalas[index].body)
    console.log("indice de botes", mboats[i].body)
    //var collision=Matter.SAT.collides(mbalas[index].body, mboats[i].body);
    
        //console.log("hubo colision", collision);
...
function keyPressed(){
  if(keyCode===DOWN_ARROW){
    ...
    console.log("matriz mbalas antes", mbalas.length)
    Matter.Body.setAngle(bulletobj.body, cannonobj.angle);
    mbalas.push(bulletobj);
    console.log("matriz mbalas después", mbalas.length)
...
function boats(){
  ...
  if(mboats.length>0){
    if(mboats[mboats.length-1]===undefined || mboats[mboats.length-1].body.position.x<width-300){
      ...
      console.log("matriz antes", mboats.length)
      mboats.push(boatobj);
      console.log("matriz después", mboats.length)
    }
